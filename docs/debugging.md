...
| CI `engine-tests` red with `Connection interrupted by cancellation requested`, a partial count (`passed=266 failed=1` of ~550), and a duration around **9m40s** | The Godot runtime goes pathologically slow on a loaded runner and its pipe is cancelled ~9m40s in. **Not** a headroom problem, and not the blamed test's fault: measured 2026-08-01 on PR #311, the SAME commit aborted at 9m48s having run 266 tests, then passed on re-run at **3m21s for all 550**. A healthy CI run is ~3m30s, so there is ~2.8x headroom — the job's 14-minute wall-clock is checkout + setup + build + import, not the test run | `gh run rerun <id> --failed`. The blamed test is whichever one was executing when the axe fell and differs every time (HumanCampaignTests, Playtest3dClickThrough, MainUiTests.ShopPanel_…, MainUiTests.ForgePanel_CraftRoundTrip all observed with identical error text) — do not "fix" that test, and do not trim the suite to buy time you already have. If it reproduces on re-run, that is new information: look for a genuinely slow call on a hot path |
