...
1. **Always:** `dotnet test sim/GameSim.Tests/GameSim.Tests.csproj -c Release --filter Category!=Balance`
   — 2245 cases on 2026-09-18 (was 1754 at the 2026-08-29 audit), ~24 s wall-clock in Release. **Never filter further than
...
| Runtime dies mid-session | `Passed!` for a partial suite | The summary counts what finished. `ENGINE_MIN_PASSED=1650` (`.github/workflows/ci.yml`) against a suite that ran 1696 at the 2026-08-29 audit — the floor is tight now, but check the current value in `ci.yml` before trusting any number written here, since this exact table row has gone stale twice already |
...
Forge minigame (`godot/scripts/minigames/ForgeMinigame.cs`, Godot-only — the sim never sees the strikes, only
the folded performance grade):

| What to check | Where | Notes |
|---|---|---|
| Grade from performance | `ForgeMinigame.PreviewGrade(int performanceGradePermille)` (static) | The only place a permille performance becomes a `QualityGrade` preview; testable in isolation |
| The path and the billet | `Path`, `ShapeXPermille`, `HeatYPermille`, `IsPumping`, `StrikesLanded` | Accumulated-clock state — no wall-clock, no engine RNG (`PathSeed` is the only seed) |
| Tempo and heat constants | `BaseRequiredStrikes` 21 / `MinRequiredStrikes` 18, `HeatDrainPermillePerSecond`, `StrikeHeatCostPermille`, `TempoPeriodSeconds` | Named constants, not magic numbers; a "cannot complete" report starts here |
| The events | `ShapingDone` (`ShapingResult`), `Struck(bool)`, `BellowsPumped`, `Cancelled` | `Completed` / `WasCancelled` flip with them; assert whichever fired, never both |
| Quality math from the grade | `QualityRoller.RollActive` (sim-side) | Bands read off `effective = grade + jitter`; the minigame computes ONLY the grade, never the band — if quality looks wrong, bisect here first |
...
| gdUnit engine suite hangs (never returns) while frames are pumped | Any `SubViewport` still rendering during the pump — 2D included, not only the deleted 3D scenes | `MountMainUi` disables rendering by default (§2b.1); a test that opts back in must disable the render-target update before pumping frames |
| Engine suite goes quiet ~880 tests in, then the session-fuse abort fires and prints a truncated `Passed!` | The Godot runtime CRASHED, not hung — `GdUnit4 Godot Runtime Test Runner ends with exit code: 134` (SIGABRT) or `139` (SIGSEGV), preceded by orphan-node warnings, then silence until the fuse. Confirmed 2026-09-15 (PR #866): two unrelated branches hit the identical orphan sequence and the same ~880-test crash point, so it's `main`'s crash, not either branch's | `run_engine_tests` reaps a stray Godot between attempts so a first crash can't poison the retry with a second concurrent runtime, and the step runs with `console;verbosity=detailed` so the last test line before the silence names the actual crash site — read that line, don't blame whichever test the next run happens to stall on |
| A rendered frame (or `tools/shoot.ps1` capture) contradicts a test that says the code is fixed | `godot/.godot/mono/temp/bin/**/GodotClient.dll` is gitignored — no `git checkout`/`fetch`/`reset` rebuilds it, so a capture can render a stale binary while stamping the CURRENT branch@sha into `build_info.txt` as a corner-label receipt. Measured 2026-09-15: a capture of `main @ d8f401ef` carried `receipt: HEAD@d8f401ef \| clean` while rendering IL 31 commits stale, and read as a merged fix not working | `shoot.ps1` now rebuilds `GodotClient.csproj` and refuses to capture a build that doesn't compile (PR #857), so its watermark is true by construction — but the transferable rule is general: **when a rendered frame contradicts a passing test, suspect the binary before the code.** Check the assembly's `LastWriteTime` against the commit before spending an investigation on the code |
| A branch's CI keeps failing on an error a merged fix already addressed | The `main-protection` ruleset does not require the branch to be up to date with `main` (`strict_required_status_checks_policy: false`), so a PR's checks run **its own branch's** copy of `.github/workflows/ci.yml` — a branch cut before a CI fix lands keeps running the broken CI forever, invisibly, until someone looks. Four PRs stalled this way in one night | `git fetch origin && git merge origin/main` into the stale branch, then re-run. A green-but-unmerged PR is inventory (rule 1) — don't leave one sitting past a CI fix without checking whether it predates it |
...
**`--frontier` infers unbuilt-ness from commit subjects and undercounts** — it has offered
already-shipped work repeatedly. Anyone dispatching a unit from it must grep for the unit's own
deliverable in source before building it; `mm-unit-builder`'s "First command, always" step exists
for exactly this reason. Never treat a frontier row as proof the work doesn't exist yet.
