...
Panels **not** backed by any scene (code-built `new`): `ProgressionPanel` (`MainUi.cs:3271`), `LessonsPanel` (`MainUi.cs:3272`), `RaidForecastBoard` (`MainUi.cs:3375`), `CommissionBoard` (`MainUi.cs:3400`), `LegendsWall` (`MainUi.cs:3409`), `CampPanel` (`MainUi.cs:3417`), system menu (`MainUi.cs:3426`, built at `MainUi.cs:4685`), `ScryingMirror` (`MainUi.cs:3518`), `PipDock` (`MainUi.cs:3523`), `MineWatch` (`MainUi.cs:3325`), `CompanionDock` (`MainUi.cs:3338`), `ObjectiveTracker` (`MainUi.cs:3440`), `TutorialFlow` (`MainUi.cs:3481`), `TutorialOverlay` (`MainUi.cs:3535`), `MentorBanner` (`MainUi.cs:3549`), `TabFade` (`MainUi.cs:3357`), `BuildStamp` (`MainUi.cs:3630`), `CounterPanel` (owned by ShopPanel, `CounterPanel.cs:14-21` embedded), `ProvenanceCard` (one per hosting panel, `ProvenanceCard.cs:16-23`). (`BestiaryPanel` was here too until #770 deleted it; `ChronicleScroll` and `AdventureTicker` were both here too; both classes are deleted — P2-MEMORY-14 and P2-MEMORY-12 — and neither's job left a code-built panel behind: see §7's `LegendsWall` row and §8 below.)
...
1. **`Layout` VBox** (`:2800`): `HudHeader` (wood panel, two rows, `:2809-3204`) → `ToastBanner` (`:3209`, hidden unless a toast is live) → **`WorldSlot`** (ExpandFill, `:3232`) containing `Town2D` full-rect (`:3239-3241`). The world sits **in layout flow** below the header — the header never occludes it (`:2791-2799`). (A `TickerWrap` row containing `AdventureTicker` used to close this stack below `WorldSlot`; P2-MEMORY-12 deleted both, and `WorldSlot` now simply keeps the full remaining height instead of splitting it with the ticker — `MainUi.cs:4014-4019`.)
2. **Overlay siblings** (draw above the layout in add order): `DrawerHost` (`:3305`), `CompanionLayer` CanvasLayer 40 with `CompanionDock` (`:3336-3339`), `TabFade` CanvasLayer 100 (`:3357`), `LedgerModal` (`:3367`), `RaidForecastBoard` (`:3375`), `BestiaryPanel` (`:3385`), `CommissionBoard` (`:3400`), `LegendsWall` (`:3409`), `CampPanel` (`:3417`), system menu (`:3426`), `ObjectiveTracker` top-right dock (`:3440-3455`), `TutorialFlow` dock below it (`:3481-3508`), `ScryingMirror` (`:3518`), `PipDock` (`:3523`), `TutorialOverlay` (`:3535`), `MentorBanner` (`:3549`), interact-prompt chip (`:3616-3625`), `BuildStamp` CanvasLayer 5 (`:3630`). (`ChronicleScroll` used to sit here too, between `BestiaryPanel` and `CommissionBoard`; P2-MEMORY-14 deleted it — `LegendsWall`, already in this list, is now also the reader for `CampaignEnded`, opening straight to its own bind page.)
...
| Tavern | `TavernPanel.cs` | "TAVERN GOSSIP", "WORK THE ROOM — IN THE COMMON ROOM" w/ per-patron Pursue rows (commission / ore / arc scene), "THE HANDSHAKE" for the pursued thread — retitled "A WORD AT THE BAR" and built ABOVE the room when the pursued thread is an arc scene (P2-PEOPLE-01) — "OUT AT THE MINE" | `Pursue` (adapter-local selection), `Pursue_Scene_{hero}` + `SceneClose_{hero}` (arc scenes — queue NO action and change no sim field), `HandshakeAccept_{hero}`/`HandshakeDecline_{hero}` (commissions), `HandshakeBuy_{hero}` (ore), `TavernHistory_{hero}_{slot}` |
...
|---|---|---|---|
| `LedgerModal` | `LedgerModal.cs` (934) | Return-Ritual auto-reveal (`MainUi.cs:888`); tray `OpenLedger` | Per-hero return cards (`LedgerQuery.ReturnCards`) in a wrapping grid (`:259`): fate line, gold purse/earned chips (`:512-515`), attribution beats (lead card sorted first — `LedgerCard_0`) with a per-beat "Ask how it happened." button into `TellingPanel` (P2-PROOF-07), warrant-save line (`:552`), "ORE OFFERED" rows with `BuyOre_{hero}_{mat}` (`:559-568`, Evening-gated mirror of `OreMarketHandlers`), tutorial tip (`:415`), first-loss block (`:292`), narrator line (`:399`), "THE RETELLING" collapsed to 8 lines (the "Full tale" toggle P2-PROOF-07 removed no longer exists), `CloseLedger` (`:932`) |
...
| `LegendsWall` | `LegendsWall.cs` (560) | tray `OpenLegends`; tavern story wall (`MainUi.cs:4343-4347`); auto on `CampaignEnded` (`MainUi.cs:1577-1580`), opening straight to the book's own bind page | "THE FALLEN" memorials (`FallenSection` `:135`) with per-memorial **Honor** buttons (`HonorMemorialAction`) and **Reforge** rows (recipe + material `OptionButton`s, `ReforgeHeirloomAction`, gate mirror `:44-50`), depths records, legend items ≥ famous-beat threshold or Signed with `Legend_{item}` provenance buttons (`:432`), `LegendsWallClose` (`:504`); the bind page (`ShowBindPage`/`RenderBindPage`, `LegendsWall.cs:555,562`) composes the ending tallies from `ChronicleComposer`, day-stamped, reopenable any number of times, with an `ExportChronicleHtml` button (`WriteHtmlExport`/`ComposeExportHtml`, `:595,630`) writing a self-contained HTML file to `user://` — replaces the deleted `ChronicleScroll` (P2-MEMORY-14) |
...
- **`AdventureTicker`** — deleted (P2-MEMORY-12, P2-OQ3). It used to be the single bottom-edge marquee, 48px/s, 3-day retention, voicing the quiet event tail (recruits, commissions expiring, rank-ups, rent/assessment lines, incidents, bounty collections). Its `FormatLine` switch moved verbatim into `LegendsWall` (`LegendsWall.cs:1286`) as the composer for the book's day pages (`RenderDayLog`/`ShowDayPage`/`DayLines`, `LegendsWall.cs:476,503,526`) — same lines, full campaign retention instead of a rolling 3-day window, queryable by day rather than ambient. The deliberate exclusions (`SupplyDelivered`, `MarketShareShifted`'s non-idle direction, `TariffApplied`) now live as a comment block in that same switch, `LegendsWall.cs:1421-1433`.
...
The one roster of addressable surfaces (`Surfaces` `:89-123`): 10 drawer ids + Ledger/Commissions/Legends/Camp/Forecast/Mirror/Chronicle/Pip/Docket (Bestiary until #770 deleted it), each with a content-root resolver and a declared `WayIn` anchor. **Surfaces declared to have no live way in** (`:42-62`): Heroes (only a roaming hero click), Chronicle (auto-only), Pip (ambient). Camp's way-in is the `AdvancePhase` bell (reopen path, `:107-111`); Mirror's is `WatchButton` (`:113-115`); Docket's is a PanelControl inside Forge (`:119-122`).
...
| Item | Evidence |
|---|---|
| **The Bestiary was unreachable in real play — resolved by deletion (#770, P2-SCREEN-14).** The panel, its 16 monster portraits' bestiary route, and `OnInteriorHotspotActivated("Bestiary")` are gone; no station in `InteriorLayout2D.Rooms` ever named Action "Bestiary", and the dormant Emberfall Foundry's only preview went with it |
...
| **`MarketShareShifted`** event (`Events.cs:214`) | **Re-evidenced, no longer 0.** 1 real reader: `LegendsWall.cs:1418` (`MarketShareShifted e when e.RivalGained`), the idle-day direction only, added by P2-HONEST-23 — the claw-back direction stays a deliberate exclusion (`LegendsWall.cs:1424`, formerly the comment at `AdventureTicker.cs:255`, deleted with that class) | The idle-day half now speaks once in the book's day pages ("You were not at the anvil today. The rival's stall was."); the field this event reports on, `RivalMarketSharePermille`, also gained a reader since this row was first written (`ShopPanel.cs:620`'s Rival Edge gauge, P2-HONEST-17) — the row above this one (`RivalMarketSharePermille`, "0" readers) has not been re-checked as part of this pass and may itself now be stale |
| **`TariffApplied`** event (`Events.cs:130`) | **Re-evidenced, no longer 0.** 1 real reader: `MainUi.cs:1617` (`state.EventLog.OfType<TariffApplied>().Any()`), gating the one-time "the-tariff-fork" mentor line (U40, #648 — predates this row). The deliberate day-page exclusion is separate and still stands: `LegendsWall.cs:1428-1433`, formerly the comment at `AdventureTicker.cs:257-263`, deleted with that class | The faction discount is voiced via the ore offer's price + `FactionStandingShifted`, and once via the tariff-fork lesson; the per-purchase delta itself still never renders as a running line |
