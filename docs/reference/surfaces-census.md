...
`godot/scripts/panels/` holds 24 files (confirmed by directory listing). 22 are surfaces, covered across this section, §7, and §8: `ForgePanel`, `ShopPanel`, `CounterPanel`, `HeroesPanel`, `TavernPanel`, `DepthsPanel`, `BountyPanel`, `DemandPanel`, `HeroPanel`, `ProgressionPanel`, `LessonsPanel` (this section); `LedgerModal`, `TellingPanel`, `RaidForecastBoard`, `CampPanel`, `ScryingMirror`, `CommissionBoard`, `LegendsWall`, `PledgePanel`, `ProvenanceCard` (§7); `MineWatch`, `DelveStage` (§8). The remaining 2 are not surfaces: `SimPanel.cs` (the abstract base every panel above inherits — binds one `SimAdapter`, owns the Bind/Refresh/Clear lifecycle, exposes the `UiKit` widget passthroughs) and `PanelGraveyard.cs` (a static node registry `SimPanel.Clear` hands every detached subtree to, so a host that never finishes a frame — an engine test — still eventually frees it; invisible in play, where the deletion queue flushes every frame regardless).
...
- **`TickCuePriority.cs`** (`godot/scripts/audio/`, new): the "one cue per tick, worst news first" ranking table `MainUi.SoundTheTick` reads from — pulled out of that method's own inline logic into a dedicated, named priority list.
- **`MixBudget.cs`** (`godot/scripts/audio/`, predates this pass — unchanged): a per-category volume/ducking budget `AudioDirector` reads alongside `UiSettings`' own faders.
...
- **`MentorBanner`** (`MentorBanner.cs`): the shared "Bryn speaks" surface — **no longer a centered modal card.** P2-ONBOARD-06 (§11.15) re-anchored it `BottomLeft` (`:197`) as a docked box, widened for beat 0's five paragraphs: `DockWidth = 640f`, `DockHeight = 500f` (`:112-114`, up from an earlier fixed 440px wrap width), `CardBodyWidth = 600f` (`:134`, the card centers inside that dock), `DockMargin = 16f`. One "Got it" button (`MentorBannerDismiss`, `:250`), **no timer ever** — shown "until the player presses 'Got it,' however long that takes." Rank-ordered queue (Lesson < Act), cap 4 with lowest-rank eviction, preempt inserts the displaced line at its rank's front, anchors travel with lines (`CurrentAnchor`, `:312`), full queue persisted through `TutorialFlow`.
...
- **`MentorCorpus.cs`** (`godot/scripts/ui/`, new, 168 lines): Bryn's WHOLE authored corpus, consolidated into one table. Before this file, every fixed line she speaks was retyped live across a dozen call sites (`MainUi.cs`/`ForgePanel.cs`/`ShopPanel.cs`/`CommissionBoard.cs`/`CounterPanel.cs`/`LegendsWall.cs`/`RaidForecastBoard.cs`/`PledgePanel.cs`/`ProgressionPanel.cs`/`TutorialFlow.cs`) — a copy that could silently drift from the words that actually render, and several fixed lines (the tariff-fork lesson, the demand-board explainer, both `LossVoiceLine`/`GraduationBeatTextFor` variants) were never checked against her never-orders register at all as a result. Every fixed line is now a named `const string` here, or forwarded here by symbol from its production file, so the tests and the screen can never disagree.
- **`MentorIdleVoice.cs`** (`godot/scripts/ui/`, new, 67 lines): what Bryn says when the apprenticeship is done and the player presses her anyway (P2-MEMORY-05, "the idle line varies"). Before this file, `MainUi.OnStationActivated` answered every such press with the same fixed `MentorVoice.RestingLine`, forever. This reuses `ObjectiveAdvisor.Suggest` — the same sim-decided, state-derived reason `ObjectiveTracker`'s own HUD chip already renders — so her idle line varies exactly when there is something real to say and never invents a fact; deterministic by construction (no RNG, no wall clock).
- **`FirstTouchLessons`** (`FirstTouchLessons.cs`): the generic once-ever-per-id engine (anti-nag pin), persisted, consumed via `TutorialFlow.ConsumeFirstTouch`. **27 first-touch ids** exist in the live `FirstTouchTitles` table (`LessonsPanel.cs:49-92`, recounted — up from "~20" as ForgePanel/Shop/Commissions/Forecast/Legends/**Pledge** each added one, including `PledgePanel.PledgeLessonId` "the-pledge" → "What the wall costs"), across MainUi (first-morning, read-only-surfaces, docket, quick-travel, second-profession, refusal-x3, idle-help), ForgePanel (material-ceiling, act1/act2, brew/assembly/tanning, talents, foundry-four-verbs, the-mark-read — the one preempting lesson), Shop/Commissions/Forecast/Legends/Progress dilemma lessons, and PledgePanel's own. "the-proof-taught" was removed from this table specifically — it moved to the dormant Proof act-voice row below (`ConsumeProofBeat`), since it fires off a settled sim fact rather than a first open.
...
| `tutorial_flow.json` / `clock_settings.json` corrupt | fail-soft to fresh defaults (`TutorialFlow.cs:3814`, `MainUi.ClockSettings.LoadAutoAdvance` `:6043-6061`) | quiet by design |
| Ground atlas missing | 2-tile flat-color fallback (`Town2D.cs:2863-2872`) | quiet, deliberate |
| `ShortcutMap`/`MinigameInput` unregistered action | renders "?" instead of a key label (`ShortcutMap.cs:166-179`, `MinigameInput.cs:76-90`) | visibly wrong, per design |
| `BuildStamp` unreadable build info | "dev (unstamped)" (`BuildStamp.cs:35`) | visible |
...
| The objective card's right edge overhangs the window by 6px — measured, left deliberately (`MainUi.cs:4229-4231`) | known cosmetic |
| `HeaderBudgetPx = 175` pins the header height (`MainUi.cs:111`); `StatChipsWrap` (`:3507`) / `TimelineWrap` (`:3563`) exist because HBox minimum widths repeatedly inflated the whole layout past 1152px. **`TickerWrap` no longer exists** — it wrapped the now-deleted `AdventureTicker` (P2-MEMORY-12) and was removed with it; the header is 3 rows now, not 2 (§3.3) | the wrap pattern is the standing defense; any new header child added outside a wrap re-opens it |
| No stretch mode + fixed 1152×648 reference (`project.godot:22-23`): all layout tuning assumes this size; `Town2D.CanvasShrink` is the only resolution-aware piece | UI at other window sizes relies on clamps, not design |
| The drawer claims a fixed 600 of 1152px (`DrawerHost.cs:46`, `DrawerWidth = 600f`, unchanged); the objective card `DockWidth = 320f` (`ObjectiveTracker.cs:43`); MentorBanner is no longer a centered 440px card — now a bottom-left dock, `DockWidth = 640f`/`CardBodyWidth = 600f` (`MentorBanner.cs:112-134`, §10.7) | fixed widths, fine at reference size; the MentorBanner resize is a real change since the old base |
...
| The tutorial dock's position is measured off the objective card, clamped + internally scrolled (`DockObjectiveHorizontally`, `MainUi.cs:5858`) after two magic-offset regressions | derived, no longer a knob |
