...
| `2026-08-02-005-feat-shell-and-audio-plan.md` | **Shell + audio wave**, from the owner's post-#340 playtest: the HUD stops eating the world (mine reachable, tested), a real app shell (title menu, honest Continue, Esc/system menu, fullscreen, save-on-quit), exactly one launcher, and the audio defect fixes (false bell on immediate actions, night-boost static, vigil noise, bellows hold) with GPU regeneration deferrable. |
...
| `2026-08-02-004-feat-world-and-interiors-plan.md` | **Fourth-playtest wave**: interiors for market/tavern/gatehouse on the forge's table-row pattern, profession-dressed workshop (shared shell, stations swap), watch re-hosted onto the animated `MineWatch`/`DelveStage` surface, staged hero return, real night darkness, hero-sprite round-3 candidates. Starts after PR #349 merges. |
