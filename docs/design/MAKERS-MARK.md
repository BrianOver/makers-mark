...
**Venue routing** (`Venues/VenueRouter.cs:58-125`): bounty parties always go to the Mine (bounties are structurally Mine-scoped — `ExpeditionSystem.cs:57-66`); bounty-free parties are routed draw-free by band (highest reached `EntryPower` wins), then queue length, then venue id. Live rotation = Mine, Gloomwood, Sunken Crypt, **Emberfall** (`Venues/VenueRegistry.cs:59-64`). Emberfall went live with the forward ladder (L4) as rank 2, the ladder's endgame rung, reachable only by graduating Gloomwood; its backdrop and all five monster portraits landed before the flip, so the guard that fails any live venue falling back to a placeholder tile passes on real art. This paragraph described it as dormant and art-less for long enough that the footer's "Appendix A wins on disagreement" rule was pointing readers at the stale half.
