...
/// Evening buy of <see cref="FactionDefinition.RiseStep"/>; for Deepvein 2 + 5 = 7 &lt; 10, enforced
/// by <c>FactionConformanceTests</c>). So a standing hovering at the boundary that drifts DOWN in the
/// Morning and is bought back UP the same Evening in a SINGLE buy cannot cross both boundaries in one
/// day-cycle. A faction supplies several ores, though, so MULTIPLE buys in one Evening can move
/// standing k·RiseStep and cross the far boundary — <c>GossipGenerator</c> collapses any contradictory
/// same-faction "cooled + favored" pair in a batch to silence, the defense-in-depth backstop to this
/// deadband. A move only ever crosses the boundary that lies in its path.</para>
///
