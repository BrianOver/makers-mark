...
/// <c>Visible</c> to false — only <c>Disabled</c>, with <see cref="Gate.ClosedReason"/> as its
/// tooltip while closed. The player should see what is coming and why it is closed, not wonder
/// whether a seventh icon exists at all.</para>
///
...
/// cref="TutorialFlow.ProofBeatAnchor"/> is the row that later landed on it — see <see
/// cref="TutorialAnchorKind.PanelControl"/>'s own doc). <c>MainUi.SurfaceEffectivelyOpen</c>
...
public static class SurfaceUnlocks
{
    ...
    /// vocabulary).
    ///
    /// <para>P2-HONEST-02: one string used to serve both moments — the closed tray button's tooltip
    /// AND the one-line arrival toast fired the instant the gate opens — under the theory that a
    /// present-tense sentence could read correctly both ways. It could not: a closed reason's own
    /// second clause names the ABSENCE the gate is waiting on ("nothing's come home yet to read",
    /// "an empty board teaches nothing"), and that absence is exactly what the arrival toast's
    /// triggering event just ended — reusing it produced a toast that announces a surface is open
    /// while, in the same breath, insisting the thing that opened it hasn't happened. <see
    /// cref="ClosedReason"/> and <see cref="OpenedReason"/> now carry their own sentence each,
    /// written for their own moment, and neither is ever substituted for the other.</para></summary>
    public readonly record struct Gate(string SurfaceId, string ClosedReason, string OpenedReason, Func<GameState, bool> Predicate);
    ...
    [
        new("Ledger",
            "Opens once a party has departed the Mine — nothing's come home yet to read.",
            "a party's departed for the Mine — the Ledger will have their story once they're back.",
        ...
        new("Forecast",
            "Opens once you reach an Evening — it forecasts tomorrow, so day 1 has nothing to say yet.",
            "evening's here — tomorrow's raid is worth a look.",
            state => state.Day > 1 || state.Phase == DayPhase.Evening),

        new("HeroCards",
            "Opens once you've sold something to a hero — a stranger becomes a customer.",
            "you've made your first sale — a stranger's a customer now.",
            state => state.EventLog.OfType<ItemSold>().Any(e => e.FromPlayerShop)),

        new("Commissions",
            "Opens once a hero posts a commission — an empty board teaches nothing.",
            "a hero's posted a commission — the board's no longer empty.",
            state => state.EventLog.OfType<CommissionPosted>().Any()),

        new("Demand",
            "Opens once a hero's passed on your goods — the board's lead section is pass reasons.",
            "a hero's passed on your goods — worth reading why.",
        ...
        new("Legends",
            "Opens once your work has changed a fate — or the town has someone to remember.",
            "your work's changed a fate, or the town's got someone to remember.",
        ...
        new("Progress",
            "Opens once a bounty's been paid — the same moment a second profession opens up.",
            "a bounty's been paid — a second profession's opened up.",
    ...
    /// (always open). <c>MainUi</c> reads <see cref="Gate.ClosedReason"/>/<see
    /// cref="Gate.OpenedReason"/> off this row for the closed-tooltip and arrival-toast text
    /// respectively, so the client never drifts apart from <see cref="Gates"/>' own single
    /// declaration.</summary>
